...
def get_sentinel2_patch(transform, processed_bands, footprint, patch_size, s2_prod) :
    """
...
def crop_and_pad_arrays(data, ul_row, lr_row, ul_col, lr_col):
    ...
    Returns:
    - data: 2d array, cropped (and padded) ALOS HH data.
    """

    # Get the dimensions of the arrays
    alos_height, alos_width = data.shape
    ...
    if ul_row < 0 or lr_row >= alos_height or ul_col < 0 or lr_col >= alos_width:
        ...
        padded_data[start_row : end_row, start_col : end_col] = data[max(0, ul_row) : min(alos_height, lr_row + 1), max(0, ul_col) : min(alos_width, lr_col + 1)]
...
def initialize_results(CH_flag) : # TODO modify
    """
    ...
    return s2_data, gedi_data, ch_data


def update_results(s2_data, gedi_data, ch_data, s2_footprint_data, gedi_footprint_data, ch_footprint_data) :
    """
    ...
    for placeholder, new_data in zip([s2_data, gedi_data, ch_data], [s2_footprint_data, gedi_footprint_data, ch_footprint_data]) :
...
def save_results(s2_data, gedi_data, ch_data, tile_name, chunk_size, file) : # TODO modify
    """
...
def init_h5_group(file, tile_name, patch_size, chunk_size, CH_flag) : # TODO modify
    """
    ...
    Sentinel-2, Sentinel-1 and GEDI data. In particular, this is the structure of the datasets:
		> (group) 32TNN
                    ...
                    # TODO : replace CH everywhere in this script by :
                    > (group) ICESat
...
def load_CH_data(path_ch, tile_name) : # TODO modify
    """
