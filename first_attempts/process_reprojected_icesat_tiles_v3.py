...
def setup_parser():
    ...
    # Paths arguments
    parser.add_argument("--tilenames", type = str, required = False, 
                   help = "Path to the .txt file listing the tiles to consider.", default = 'code/tile_names.txt') 
    parser.add_argument('--path_shp', help = 'Path to the Sentinel-2 index shapefile.', default = local_path_shp)
    parser.add_argument("--output_path", type = str, required = False, 
                   help = "Path to the folder where the tiles will be downloaded.", default = 'reprojected data')

    # Arguments for the procedure
    parser.add_argument("--year", type = int, required = False, help = "Year for which to download the tiles.", default = 2020)
...
def convert_to_WGS84(dataset, x, y,) :
...
def get_tiles_from_coordinates(lat_min_s2, lat_max_s2, lon_min_s2, lon_max_s2, meridian_flag) :
    """
    Get the ICESat-2 tiles that intersect the given coordinates.
    ...
    - meridian_flag: bool, whether the coordinates cross the meridian

    Returns:
    - list of str, names of the ICESat-2 tiles
    """
    tiles = []
    data_folder = 'reprojected data'  # Replace with the actual path to the data folder
    for file_name in os.listdir(data_folder):
        if file_name.endswith('.tiff'):
            ...
            with rs.open(file_path) as dataset:
                ...
                # # Convert the bounding box coordinates
                # lat_min_icesat, lon_min_icesat = convert_to_WGS84(dataset, dataset.bounds[0], dataset.bounds[1])
                # lat_max_icesat, lon_max_icesat = convert_to_WGS84(dataset, dataset.bounds[2], dataset.bounds[3])

                lat_min_icesat = dataset.bounds[1]
                lon_min_icesat = dataset.bounds[0]
                lat_max_icesat = dataset.bounds[3]
                lon_max_icesat = dataset.bounds[2]
                
                # if file_name == 'boreal_agb_202302061675660693_2656.tiff':
                #     print(lon_min_icesat, lat_min_icesat, lon_max_icesat, lat_max_icesat)
                #     print(lon_min_s2, lat_min_s2, lon_max_s2, lat_max_s2)
                # Define the bounding boxes
                bbox_icesat = (lon_min_icesat, lat_min_icesat, lon_max_icesat, lat_max_icesat)
                bbox_s2 = (lon_min_s2, lat_min_s2, lon_max_s2, lat_max_s2)

                # Check for overlap
                if not (bbox_icesat[2] < bbox_s2[0] or  # bbox_icesat is to the left of bbox_s2
                        bbox_icesat[0] > bbox_s2[2] or  # bbox_icesat is to the right of bbox_s2
                        bbox_icesat[1] > bbox_s2[3] or  # bbox_icesat is above bbox_s2
                        bbox_icesat[3] < bbox_s2[1]):   # bbox_icesat is below bbox_s2
...
def mosaic_tiles(tiles, band, year, path_ICESat_data) :
    ...
    to_mosaic = []
    #TODO: figure out what is going on here: is the location encoded in the name?
    for tile in tiles :
        src = rs.open(join(path_ICESat_data, tile))
...
def save_tile(s2_tname, year, bands_data, output_path) :
    ...
    fname = f'ICESat2_{s2_tname}.tif'
    with rs.open(join(output_path, fname), 'w', **ICESat_meta) as dst:
        for band_id, (band_name, band_data) in enumerate(bands_data.items()):
            print(np.isnan(band_data['mosaic']).any())
            print(np.isinf(band_data['mosaic']).any())
            band_data['mosaic'] = np.nan_to_num(band_data['mosaic'], nan=0.0, posinf=100, neginf=-100)
...
def process_ICESat_tiles(s2_tname, s2_geom, year, path_ICESat_data) :
    ...
    try:
        ...
        if len(ICESat_tiles) == 0 : print(f'No ICESat tiles found for {s2_tname}.')
        else:

            print(f'>> Got {len(ICESat_tiles)} ICESat tile(s).')
            print(ICESat_tiles)
            ...
            for band in ['aboveground_biomass_density (Mg ha-1)', 'standard_deviation'] :
                ...
                with rs.MemoryFile() as memfile:
                    with memfile.open(driver = 'GTiff', height = mosaic.shape[1], width = mosaic.shape[2], count = 2, 
                                        dtype = mosaic.dtype, crs = mosaic_crs, transform = mosaic_trans) as mosaic_vrt:
                        mosaic_vrt.write(mosaic)
                        print(f'Mosaic bounds: {mosaic_vrt.bounds}')
                        print(f'Sentinel-2 tile bounds: {s2_geom.bounds}')
...
if __name__ == "__main__":
    ...
    assert len(tile_names) == len(tile_geoms), "The number of tile names and geometries is not the same."
    # print(tile_names, tile_geoms)

    tiles_num = len(tile_names)
    print(f'Processing {tiles_num} tile(s)...')

    # for tile_name in tile_names :
    #     print(get_CRS_from_S2_tilename(tile_name))

    # process_ICESat_tiles(tile_names[22], tile_geoms[22], year, path_ICESat_data)
