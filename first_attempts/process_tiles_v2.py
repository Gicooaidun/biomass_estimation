...
def setup_parser():
    ...
    parser.add_argument("--output_path", type = str, required = False, 
                   help = "Path to the folder where the tiles will be downloaded.", default = 'data')
...
def get_tiles_from_coordinates(lat_min_s2, lat_max_s2, lon_min_s2, lon_max_s2, meridian_flag) :
    ...
    tiles = []
    data_folder = 'data'  # Replace with the actual path to the data folder
    ...
    try:
        ...
        if len(ICESat_tiles) == 0 : print(f'No ICESat tiles found for {s2_tname}.')
            ...
            for band in ['aboveground_biomass_density (Mg ha-1)', 'standard_deviation'] :
                ...
                print(get_CRS_from_S2_tilename(s2_tname))
                print(s2_geom)
                print(mosaic_crs)
                
                # Transform the mosaic to the same CRS as the Sentinel-2 tile
                dst_crs = get_CRS_from_S2_tilename(s2_tname)
                mosaic_crs = dst_crs

                print(mosaic_trans)
                mosaic, mosaic_trans = rs.warp.reproject(source=mosaic, destination=mosaic, src_transform=mosaic_trans, src_crs=mosaic_crs, dst_crs=dst_crs, resampling=rs.warp.Resampling.nearest)
                print(mosaic_trans)
