...
def setup_parser() :
    ...
    parser.add_argument('--path_bm', help = 'Path to the BM (ICESat) data.', default = '/scratch2/biomass_estimation/code/notebook/cropped_mosaic_no_nan')
    ...
    with h5py.File(join(output_path, fname), 'a') as file :
        ...
        for s2_prod, footprints in groups :
            ...
            for footprint in footprints.itertuples() :
                ...
                s2_footprint_data = get_sentinel2_patch(transform, processed_bands, footprint, patch_size, s2_prod, debug = True)
                ...
                with rs.open(f'/scratch2/biomass_estimation/code/patches/debug/s2_{tile_name}_{footprint_idx}.tif', 'w', **s2_meta) as dst:
                    ...
                    dst.write(s2_footprint_data['bands'][7], 1)
                ...
                if BM_flag: 
                    ...
                    # print(bm_footprint_data['bm'])
                    meta = {'driver': 'GTiff', 'height': patch_size[0], 'width': patch_size[1], 'transform': s2_footprint_data['patch_transform'], 'crs': crs_2, 'count' : 2, 'dtype' : np.float32}
